...
export function useCachedData(): UseCachedDataReturn {
  ...
  const loadFromCache = useCallback(async () => {
    ...
    setError(null);
    try {
      const [paymentsResp, lineItemsResp, expectationsResp] = await Promise.all([
        cacheApi.getPayments(),
        cacheApi.getLineItems(),
        cacheApi.getExpectations(),
      ]);

      const paymentsData: CachedPaymentRow[] = paymentsResp.data || paymentsResp || [];
      const lineItemsData: CachedLineItemRow[] = lineItemsResp.data || lineItemsResp || [];
      const expectationsData: CachedExpectationRow[] = expectationsResp.data || expectationsResp || [];
  ...
  // Prisma DateTime fields require full ISO string, even for @db.Date columns
  const toDate = (d: string | null | undefined): string | null =>
    d ? new Date(d.includes('T') ? d : d + 'T00:00:00.000Z').toISOString() : null;
  ...
  const saveToCache = useCallback(async (payments: Payment[], expectations: Expectation[]): Promise<boolean> => {
    ...
    try {
      ...
      await cacheApi.deleteAllLineItems();
      await cacheApi.deleteAllPayments();
      await cacheApi.deleteAllExpectations();
      ...
      if (payments.length > 0) {
        const paymentRows = payments.map(p => ({
          ...
          periodStart: toDate((p as any).dateRangeStart),
          periodEnd: toDate((p as any).dateRangeEnd),
          status: p.status,
          reconciledAmount: p.reconciledAmount,
          remainingAmount: p.remainingAmount,
          notes: p.notes || null,
          zohoRecordId: p.zohoId || p.id,
        }));

        await cacheApi.bulkUpsertPayments(paymentRows);
        ...
        const lineItemRows = payments.flatMap(p =>
          p.lineItems.map(li => ({
            ...
            planReference: li.planReference,
            adviserName: li.adviserName || null,
            amount: li.amount,
            feeCategory: li.feeCategory,
            status: li.status,
            matchedExpectationId: li.matchedExpectationId || null,
            matchNotes: li.matchNotes || null,
            zohoRecordId: li.zohoId || li.id,
          }))
        );

        if (lineItemRows.length > 0) {
          await cacheApi.bulkUpsertLineItems(lineItemRows);
        }
      }

      // Insert expectations in batches of 500
      if (expectations.length > 0) {
        const expectationRows = expectations.map(e => ({
          id: e.id,
          providerName: e.providerName,
          clientName: e.clientName,
          planReference: e.planReference,
          adviserName: e.adviserName || null,
          expectedAmount: e.expectedAmount,
          calculationDate: toDate(e.calculationDate),
          feeCategory: e.feeCategory,
          status: e.status,
          allocatedAmount: e.allocatedAmount,
          remainingAmount: e.remainingAmount,
          zohoRecordId: e.zohoId || e.id,
        }));

        const BATCH_SIZE = 500;
        for (let i = 0; i < expectationRows.length; i += BATCH_SIZE) {
          const batch = expectationRows.slice(i, i + BATCH_SIZE);
          await cacheApi.bulkUpsertExpectations(batch);
        }
      }
  ...
  const clearCache = useCallback(async (): Promise<boolean> => {
    try {
      await cacheApi.deleteAllPendingMatches();
      await cacheApi.deleteAllLineItems();
      await cacheApi.deleteAllPayments();
      await cacheApi.deleteAllExpectations();
  ...
  const updateExpectationStatus = useCallback(async (
    ...
    try {
      ...
      await cacheApi.updateExpectation(expectationId, updateData);
      return true;
    } catch (err) {
  ...
  const updatePaymentStatus = useCallback(async (
    ...
    try {
      await cacheApi.updatePayment(paymentId, { status, reconciled_amount: reconciledAmount, remaining_amount: remainingAmount });
      return true;
    } catch (err) {
  ...
  const savePendingMatch = useCallback(async (match: {
    ...
    try {
      await cacheApi.createPendingMatch({
  ...
  const getPendingMatches = useCallback(async () => {
    try {
      const resp = await cacheApi.getPendingMatches();
      const rows = resp.data || resp || [];

      console.log(`[Cache] Fetched ${rows.length} pending matches`);

      return rows
        .filter((m: any) => !m.synced_to_zoho)
        .map((m: any) => ({
          id: m.id,
  ...
  const markMatchesSynced = useCallback(async (matchIds: string[]): Promise<boolean> => {
    try {
      ...
      for (let i = 0; i < matchIds.length; i += CHUNK_SIZE) {
        ...
        await Promise.all(
          chunk.map(id => cacheApi.updatePendingMatch(id, { synced_to_zoho: true, synced_at: new Date().toISOString() }))
        );
      }
      return true;
    } catch (err) {
